...
pub fn execute_command(
    ...
    match (request_method, request_target) {
        ("GET", "/") => get_cmd(stream),
        ("GET", target) if target.starts_with("/echo/") => get_echo(parsed_request, stream),
        ("GET", "/user-agent") => get_user_agent(parsed_request, stream),
...
fn get_cmd(stream: &mut TcpStream) -> Result<()> {
    ...
    response.push_str(template::STATUS_200);
    response.push_str(template::HEADER_END);
...
fn get_echo(parsed_request: ParsedRequest, stream: &mut TcpStream) -> Result<()> {
    if let Some(echo_content) = parsed_request.target.strip_prefix("/echo/") {
        let mut response = String::new();
        response.push_str(template::STATUS_200);
        response.push_str(template::CONTENT_TYPE_PLAIN);
        response.push_str(&template::content_length(echo_content.len()));
        response.push_str(template::HEADER_END);
...
fn get_files(
    ...
    if let Some(file_path) = parsed_request.target.strip_prefix("/files/") {
        ...
        match std::fs::read(&full_path) {
            ...
            Err(_) => {
                let mut response = String::new();
                response.push_str(template::STATUS_404);
                response.push_str(template::HEADER_END);
    ...
    let mut response = String::new();
    response.push_str(template::STATUS_200);
    response.push_str(template::CONTENT_TYPE_OCTET_STREAM);
    response.push_str(&template::content_length(file_bytes.len()));
    response.push_str(template::HEADER_END);
...
fn post_files(
    ...
    let mut response = String::new();
    response.push_str(template::STATUS_201);
    response.push_str(template::HEADER_END);
